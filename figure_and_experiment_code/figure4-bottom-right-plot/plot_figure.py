...
for i in range(len(config['results'])):
    ...
    plt.gca().annotate(r'$\sum_{\ell}\|W_\ell\|^2 \propto(n_{\mathtt{train}})^{'+str(a)+'}$', (x,y), xytext=(x/3, y*1.25))
    ...
    # plt.scatter(df_small['n_tr'], df_small['norm'],color=colorsL[i], marker='.', label=r'$\mathrm{HLW} = $' + str(eval(run_config['hidden_layer_sizes'])[0]))
    plt.scatter(df_small['n_tr'], df_small['norm'],color=colorsL[i], marker='.')

plt.legend()
plt.title(r"Squared norm vs $n_{\mathtt{train}}$")
plt.xscale('log')
plt.yscale('log')
plt.xlabel(r'$n_{\mathtt{train}}$')
plt.ylabel(r'$\sum_{\mathrm{layers}} \|$ weights $\|^2$')
